...
Status: **built 2026-08-06** (phases 0, 1, 1b, 2 - see docs/PACKAGING.md for the
build recipe, what was verified vs assumed, and the frozen-build decisions).
Deviations from this plan, with reasons:

- **Feed detection folded into `/api/setup/validate-keys`** (no separate
  detect-feed endpoint): one round trip, and the keys stay in one place.
- **No `setup_done` flag in config.json.** First-run truth is derived from
  bot/.env alone (missing / placeholders / empty keys). A second flag would be
  a second source of truth waiting to disagree with the first.
- **No system tray.** pywebview has no tray API; pystray would add a dependency
  plus a second event loop. `/api/shell/quit` covers Quit, Re-scan is already a
  button in the UI, and show/hide is the taskbar's job. Revisit only if Tauri.
- **Splash is generated by app.js + a style.css block**, not a fetched
  static/splash.html fragment - one less round trip, same overlay, same skips.
- **Frozen child processes** run through an argv dispatch shim in shell.py
  (the exe emulates `python run_bot.py|radar.py`). The engine STAYS a
  subprocess: the supervisor's crash-restart, the orphan pid-kill and the
  timeout-killable radar are process-level safety properties that an
  in-process engine would silently delete.
